...
class stage01_resequencing_gd_io(stage01_resequencing_gd_query,
    ...
    def import_resequencingData_add(self, filename, experiment_id, sample_name):
        ...
        #metadata, mutations, evidence, validation = self.import_gd(filename, experiment_id, sample_name)
        ## add data to the database:
    ...
    def export_dataStage01ResequencingMutationsSeqChanges_js(self,
                analysis_id_I,
                mutation_id_exclusion_list=[],
                frequency_threshold=0.1,
                max_position=4630000,
                data_dir_I="tmp"):
        ...
        for sample_name_cnt,sample_name in enumerate(sample_names):
            # query mutation data:
            mutations = [];
            mutations = self.get_mutations_experimentIDAndSampleName_dataStage01ResequencingMutationsSeqChanges(experiment_ids[sample_name_cnt],sample_name);
            for end_cnt,mutation in enumerate(mutations):
                ...
                #generate an html reprentation of the sequence change
                dna_ref_html,dna_new_html,rna_ref_html,rna_new_html,peptide_ref_html,peptide_new_html = genomediff._make_mutationSeqChangeHTML(mutation);
                tmp['dna_sequence_ref']=dna_ref_html;
                tmp['dna_sequence_new']=dna_new_html;
                tmp['rna_sequence_ref']=rna_ref_html;
                tmp['rna_sequence_new']=rna_new_html;
                tmp['peptide_sequence_ref']=peptide_ref_html;
                tmp['peptide_sequence_new']=peptide_new_html;
                #peptide length
                tmp['peptide_length_ref']=len(mutation['peptide_sequence_ref']);
                tmp['peptide_length_new']=len(mutation['peptide_sequence_new']);
                data_O.append(tmp);
        # dump chart parameters to a js files
        data1_keys = [
                    'experiment_id',
        ...
        formparameters_O = {'htmlid':'filtermenuform1','htmltype':'form_01',"formsubmitbuttonidtext":{'id':'submit1','text':'submit'},"formresetbuttonidtext":{'id':'reset1','text':'reset'},"formupdatebuttonidtext":{'id':'update1','text':'update'}};
        formtileparameters_O.update(formparameters_O);
        # make the DNA html object
        # make the RNA html object
        # make the Peptide html object

        # make the table parameter object
        tableparameters_O = {"tabletype":'responsivetable_01',
                    'tableid':'table1',
                    "tablefilters":None,
                    'ntablerows':100000.0,
